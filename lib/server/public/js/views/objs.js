...
App.Views.ObjsActions = Backbone.View.extend({
  template: _.template('<div class="switch"></div>\
<ul>\
...
App.Views.ObjsRow = Backbone.View.extend({
  ...
  onClick: function(e){
    e.preventDefault();
    e.stopPropagation();
    App.router.navigate('objects/' + this.person.get('id'), {trigger: true});
  },
  render : function() {
    var data = this.person.toJSON();
